...
const { DELETE_POST, ADD_LIKE } = require("../actions/actions.jsx");
        ...
        <div className="blog_posts">
            {
                props.posts.map((post) =>
                    <div className="card mb-3" key={post.id} >
                        ...
                        <button className="details" onClick={() => { props.history.push("/post/" + post.id)}}>Читать подробнее</button> 
                        <span>{post.like}</span>
                        <span>{numberOfComments(post.id)} комментариев</span>
                        <div className="card-footer text-body-secondary">
                            <button type="button" className="btn btn-danger" data-id={post.id} onClick={handlePostDelete}>Удалить</button>
                        </div>
                        <div className="card-footer text-body-secondary">
                            <button type="button" className="btn btn-danger" onClick={() => { props.history.push(`/post/edit/${post.id}`) }}>Редактировать</button>
                        </div>
                    </div>
                )
            }
...
module.exports = connect(mapStateToProps, mapDispatchToProps)(Block);

//props.history.push("/post/" + post.id) - history здесь, как редирект